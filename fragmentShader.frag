...
void main(void)
{
	...
	for(int i = 0; i < 1; i++){
		...
		// Ashikhmin diffuse
		vec4 ashDiffuse	= gl_LightSource[i].diffuse * vec4((vec3(1) - gl_LightSource[i].specular.rgb), 1);
		...
		vec4 ashSpecFres	= vec4(gl_LightSource[i].specular.rgb + (vec3(1.0) - gl_LightSource[i].specular.rgb)*pow(1 - dot(l, h), 5), 1);
		vec4 ashSpecular	= ashSpecNorm * pow(max(dot(n, h), 0), ashSpecExp) * ashSpecFres / ashSpecDenom;

		totalColor		+= max(ambient*0 + ashDiffuse + ashSpecular, 1) * fragColor;
	}
